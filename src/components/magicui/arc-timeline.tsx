...
interface ArcTimelineProps extends ComponentPropsWithoutRef<"div"> {
  ...
  arcConfig?: {
    ...
    /**
     * The speed of infinite rotation in degrees per second, default is 0.1
     */
...
export function ArcTimeline(props: ArcTimelineProps) {
  ...
  const {
    ...
    rotationSpeed = 0.1, // Slower default rotation speed
  } = arcConfig;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (isRotating) {
        ...
        const rotationDegrees = elapsedSeconds * rotationSpeed;
        
        setCircleContainerRotateDeg(prev => prev - rotationDegrees * 0.001);
      }
  ...
  const handleStepClick = () => {
    setIsRotating(false);
    // Find the first 2024 step
    let first2024Angle = 0;
    let found2024 = false;
    
    for (let lineIndex = 0; lineIndex < data.length; lineIndex++) {
      const line = data[lineIndex];
      if (line.time.toString().includes("2024")) {
        // Calculate angle for first step of 2024
        const count = data
          .slice(0, lineIndex)
    ...
    if (found2024) {
      setCircleContainerRotateDeg(-1 * first2024Angle);
    }
  };
    ...
    >
      <div
        ...
        className={cn(
          ...
          isRotating ? "transition-none" : "transition-all duration-500 ease-in-out"
        )}
      >
              ...
              {line.steps.map((step, stepIndex) => {
                ...
                return (
                  <div key={`${lineIndex}-${stepIndex}`}>
                    ...
                    <div
                      className={cn(
                        "absolute left-1/2 top-0 -translate-x-1/2 cursor-pointer transition-all duration-200 hover:scale-105",
                    ...
                    >
                        ...
                        <div
                          className={cn(
                            ...
                            isActive
                              ...
                              : "translate-y-[calc(100%_+_4px)] scale-100 text-[var(--icon-inactive-color,#a3a3a3)] dark:text-[var(--icon-inactive-color,#a3a3a3)] hover:scale-110",
                          )}
                        >
                          {step.icon}
                        </div>
                      ...
                      {stepIndex === 0 && (
                        <div
                          className={cn(
                            "absolute left-1/2 top-0 z-10 -translate-x-1/2 translate-y-[calc(-100%-24px)] whitespace-nowrap transition-colors duration-200",
                            isActive
